...
if __name__ == "__main__":
    ...
    if True:
        ...
        Y = model.fit_generator(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes),
    ...
    if True:
        ...
        batch_size = 2
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        H = model.fit_generator(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                steps_per_epoch=max(1, num_train//batch_size),
                validation_data=data_generator(lines[num_train:], batch_size, input_shape, anchors, num_classes),
                validation_steps=max(1, num_val//batch_size),
                epochs=400,
        ...
        print(Y.history['loss'])
        print(Y.history['val_loss'])
        ...
        plt.plot(Y.history['loss'],color='red',linewidth=1.5)
        plt.plot(Y.history['val_loss'],color='black',linewidth=1.5)
        # plt.title("Model Loss Function", fontsize=25)
        ...
        plt.xticks(fontsize=20, rotation=45)
        plt.yticks(fontsize=20)
        plt.plot(H.history['loss'],color='red',linewidth=1.5)
        plt.plot(H.history['val_loss'],color='black',linewidth=1.5)
        # plt.title("Model Loss Function", fontsize=25)
        plt.ylabel("Loss", fontsize=23)
        plt.xlabel("Epoch", fontsize=23)
        plt.legend(["Train Loss", "Validation Loss"], fontsize=20)
